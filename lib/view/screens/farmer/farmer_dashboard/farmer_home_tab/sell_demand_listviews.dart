...
import 'cards/crop_sell_card.dart';
...
class _SellDemandState extends State<SellDemand> {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        sellDemandState == 0
            ? sellCards(widget.search)
            : demandCards(widget.search),
        const SizedBox(
          height: 100,
        )
      ],
  ...
  sellCards(String search) {
    return FutureBuilder<List<SellCropDataFarmer>>(
      future: HomeScreenController.fetchDataForSell(context),
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          List<SellCropDataFarmer>? data = snapshot.data;
          return ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                return (data[index]
                    ...
                    ? CropSellCardFarmer(ctx: widget.ctx, data: data[index])
                    : Container();
              });
  ...
  demandCards(String search) {
    return FutureBuilder<List<DemandCropDataFarmer>>(
      future: HomeScreenController.fetchDataForDemand(context),
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          List<DemandCropDataFarmer>? data = snapshot.data;
          return ListView.builder(
              physics: const NeverScrollableScrollPhysics(),
              shrinkWrap: true,
              itemCount: data!.length,
              itemBuilder: (BuildContext context, int index) {
                return (data[index]
                        ...
                        .toLowerCase()
                        .contains(search.toLowerCase()))
                    ? CropDemandCardFarmer(ctx: widget.ctx, data: data[index])
                    : Container();
              });
        } else if (snapshot.hasError) {
          return Text("${snapshot.error}");
