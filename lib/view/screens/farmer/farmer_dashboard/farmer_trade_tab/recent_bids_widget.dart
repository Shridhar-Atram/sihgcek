...
class RecentBidsWidget extends StatelessWidget {
  ...
  recentBidCards(BuildContext context) {
    return FutureBuilder<List<RecentBidDataFarmer>>(
      future: fetchData(context),
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          List<RecentBidDataFarmer>? data = snapshot.data;
          return ListView.builder(
              physics: const NeverScrollableScrollPhysics(),
              shrinkWrap: true,
              itemCount: data?.length,
              itemBuilder: (BuildContext context, int index) {
                return RecentBidCardFarmer(ctx: context, cropData: data![index]);
              });
  ...
  static Future<List<RecentBidDataFarmer>> fetchData(
      BuildContext context) async {
    try {
      List<RecentBidDataFarmer> mReturnVal = [];
      ...
      await FirebaseFirestore.instance
        ...
        for (var element in element.docs) {
          mReturnVal.add(RecentBidDataFarmer.fromJson(element.data()));
        }
      });
