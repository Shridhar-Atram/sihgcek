...
class YourAuctionWidget extends StatelessWidget {
  ...
  yourAuctionWidgetCards(BuildContext context) {
    return FutureBuilder<List<FarmerAuctionData>>(
      future: fetchData(context),
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          List<FarmerAuctionData>? data = snapshot.data;
          return ListView.builder(
              physics: const NeverScrollableScrollPhysics(),
              shrinkWrap: true,
              itemCount: data?.length,
              itemBuilder: (BuildContext context, int index) {
                return YourAuctionBidCardFarmer(
                    ctx: context, farmerAuctionData: data![index]);
              });
  ...
  static Future<List<FarmerAuctionData>> fetchData(BuildContext context) async {
    try {
      List<FarmerAuctionData> mReturnVal = [];
      ...
      await FirebaseFirestore.instance
        ...
        for (var element in element.docs) {
          mReturnVal.add(FarmerAuctionData.fromJson(element.data()));
        }
      });
