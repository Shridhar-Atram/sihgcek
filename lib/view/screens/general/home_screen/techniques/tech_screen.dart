...
class _AddCropScreenFarmerState extends State<TechScreen> {
  @override
  ...
  yourAuctionWidgetCards(BuildContext context) {
    return FutureBuilder<List<TechData>>(
      future: fetchData(context),
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          List<TechData>? data = snapshot.data;
          print(data?.length);
          return ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                ...
                return GestureDetector(
                  onTap: (){Navigator.pushNamed(context, RoutePaths.techDetailScreen , arguments: {"data": data?[index]});},
                  child: Container(
  ...
  static Future<List<TechData>> fetchData(
      BuildContext context) async {
    try {
      List<TechData> mReturnVal = [];
      ...
      await FirebaseFirestore.instance
        ...
        for (var element in element.docs) {
          mReturnVal.add(TechData.fromJson(element.data()));
        }
      });
